...
int checkCollision() {
	if(pow((lt.latx - tr.tarx), 2) < 5 && pow((lt.latz - tr.tarz), 2) < 5)
		return 1;
	return 0;
}
...
void drawScene() {
	...
	drawTarget();
	strcpy(buffer, "Your Score: ");
	strcat(buffer,itoa(lt.score,scr,10));
    output(130.0f,0.0f, 0.0f,255.0f,255.0f,255.0f,GLUT_BITMAP_TIMES_ROMAN_24,buffer);
	glutSwapBuffers();
}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("A Game of Spin");
	initRendering();
	tr.tarx = rand()%128;
	tr.tarz = rand()%128;
	if(tr.tarz >= 64)
